...
def find_nearest_neighbours_stupid(x: np.ndarray, all_points: np.ndarray, n_nearest: int = 1) -> np.ndarray:
    res = []
    for point in x:
...
def test_kd_tree_query() -> None:
    for _ in range(100):
        ...
        x_train = np.array([np.random.uniform(-200, 200, size=dim) for _ in range(np.random.randint(100, 200))])
        ...
        for i in range(len(x_test)):
            for j in range(n_near):
                assert np.linalg.norm(x_test[i] - res1[i][j]) == np.linalg.norm(x_test[i] - res2[i][j])
