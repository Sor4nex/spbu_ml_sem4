...
class KDTree:
    ...
    def _build_kd_tree(self, x: np.array, leaf_size: int) -> Node | np.ndarray:
        ...
        node.split_axis = int(np.argmax([max(x[:, i]) - min(x[:, i]) for i in range(len(x[0]))]))

        #sort array by axis, whose values has the biggest range
        x = x[np.argsort(x[:, node.split_axis])]
        node.median = x[(len(x) - 1) // 2][node.split_axis] if len(x) % 2 == 0 else (x[len(x) // 2][node.split_axis] + x[len(x) // 2 - 1][node.split_axis]) / 2
    ...
    def query(self, x: np.ndarray, n_nearest: int = 1) -> np.ndarray:
        ...
        def _search_n_nearest(point: np.ndarray, node: Node | np.ndarray, n_nearest: int = 1) -> tuple[list, float]:
            ...
            res1 = _search_n_nearest(point, node.right_subtree if point[node.split_axis] >= node.median else node.left_subtree, n_nearest)
            ...
            if abs(point[node.split_axis] - node.median) < res1[1] or len(res1[0]) < n_nearest:
                res2 = _search_n_nearest(point, node.left_subtree if point[node.split_axis] >= node.median else node.right_subtree, n_nearest)
                res = res1[0] + res2[0]
                res.sort(key=lambda el: el[1])
                if len(res) > n_nearest:
                    res = res[:n_nearest]
